...
export class AccountPositionService {
  ...
  public getBalance = (playerId:number):Promise<Observable<Balance>> => {
      return this.kc.getToken().then(token => {
        ...
        headers.append('Authorization', 'Bearer ' + token);
        return this.http
          .get(this.playersUrl + playerId + '/balance', new RequestOptions({ headers: headers }))
          .map(res => res.json());
      });
  }
  ...
  public getAccountPositions = (playerId:number):Promise<Observable<AccountPosition[]>> => {
      return this.kc.getToken().then(token => {
        ...
        headers.append('Authorization', 'Bearer ' + token);
        return this.http
          .get(this.playersUrl + playerId + '/accountpositions/', new RequestOptions({ headers: headers }))
          .map(res => res.json());
      });
  }
  ...
  public getAccountHistoryForPlayer = (playerId:number):Promise<Observable<TimeEntry[]>> => {
      return this.kc.getToken().then(token => {
        ...
        headers.append('Authorization', 'Bearer ' + token);
        return this.http
          .get(this.playersUrl + playerId + '/accounthistory/' + '?summedUp=true', new RequestOptions({ headers: headers }))
          .map(res => res.json());
      });
  }
  ...
  public getAccountHistory = (timeUnit:string, maxEntries:number):Promise<Observable<HistoryEntry[]>> => {
      return this.kc.getToken().then(token => {
        ...
        headers.append('Authorization', 'Bearer ' + token);
        return this.http
          .get(this.playersUrl + 'accounthistory/' + '?summedUp=true&timeUnit=' + timeUnit + '&maxEntries=' + maxEntries, new RequestOptions({ headers: headers }))
          .map(res => res.json());
      });
  }
